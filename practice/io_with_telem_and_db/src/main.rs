...
use std::{
    thread, time::Duration, io, io::{BufWriter, BufReader, Write}, fs::File, error::Error,
};
use chrono::prelude::*;
...
//file io take Vec with TelemtetrySample as param (data)
fn export(data: &Vec<TelemetrySample>) -> Result<(), Box<dyn std::error::Error>> {
    let file = File::create("CrashScribe_Report.csv")?;
    let mut buf_writer = BufWriter::new(file);
    buf_writer.write_all("timestamp,cpu,used_memory,total_memory\n".as_bytes())?;
    for sample in data {
        writeln!(buf_writer, "{},{},{},{}",
            sample.timestamp, sample.cpu, sample.used_memory, sample.total_memory
        )?;
    }
    Ok(())
}

//get data from db, return as result of type Vec with TelemetrySample
fn get_data() -> Result<Vec<TelemetrySample>, Box<dyn std::error::Error>> {
